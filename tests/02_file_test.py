...
class TestEdifact(unittest.TestCase):
    def test_files(self):
        testfiles_dir = os.path.join(os.path.dirname(__file__), "testfiles", "original")
...
#                cleaned_file_pointer.close()
                ...
                # read reference file
                with open(os.path.join(os.path.dirname(__file__), "testfiles", "reference", basename), 'r') as ref_file:
